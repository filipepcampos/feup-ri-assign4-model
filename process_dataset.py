...
def convert_dir(input_dir, output_dir, split):
    # Copy imgs to output dir
    input_imgs = os.path.join(root_dir_path, split, "images")
    output_imgs = os.path.join(output_dir_path, split, "images")
    os.makedirs(output_imgs, exist_ok=True)
    os.system(f"cp -r {input_imgs}/* {output_imgs}")

    # Convert labels
    input_dir = os.path.join(root_dir_path, split, "labels")
    output_dir = os.path.join(output_dir_path, split, "labels")
...
output_dir_path = "../datasets/duckietown2_processed"

convert_dir(root_dir_path, output_dir_path, "train")
convert_dir(root_dir_path, output_dir_path, "valid")